...
{
    public partial class FrmPasajePremium : FrmVentaPasaje
    {
        private Bolsos valija;
        private Bolsos valija2;
        private bool bandera;

        public FrmPasajePremium(bool regionalONo)
        {
            InitializeComponent();
            ...
            comboBox_nacion.DataSource = nacionalidades;
        ...
        {
            if (banderaValija == 0 || banderaValija ==1)
            {
                if(banderaValija==1)
                {
                    banderaValija = 2;// se cargo la 2 valija
                    valija2 = new Bolsos(comboBox_valija.SelectedIndex, Equipaje.De_Bodega);
                }
                else
                {
        ...
        private void button_crearPasajero_Click(object sender, EventArgs e)
        {
            if(base.Validar()==true)
            {
                ...
                Nacionalidad nacionalidad = (Nacionalidad)comboBox_nacion.SelectedIndex;
                ...
                if (valija2 is not null)
                {
                    pasajero = pasajero + valija2;
                }
